...
outFile = "out_firstperiod_nocases_mcmc.rdata"
...
retry_wrapper = function(params)
{
  ...
  model_wrapper = function(params)
  {
    ...
    res  = calc_model_stats(params[-1], abc_param_names, params_calib_abc, params_temporal_calib_abc, state_abc, 
                            TRUE, start_date = ymd("2021-04-01"), end_date = ymd("2021-06-15"), dates = dates_abc, rescale_factors = data_means, stats_to_include = c("hosp"))
    return(res)
  }
...
abc_rej = ABC_rejection(model = retry_wrapper,
                        ...
                        use_seed = TRUE,
                        verbose = TRUE)
save(abc_rej, file = "out_firstperiod_nocases_rej.rdata")
# 
