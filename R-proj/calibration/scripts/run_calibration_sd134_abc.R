...
retry_wrapper = function(params)
{
  ...
  out_file = sprintf("~/output_%d.txt" , Sys.getpid())
  
  cat(paste(params, collapse = ", "), file = out_file, append = TRUE, sep = ",")
  ...
  model_wrapper = function(params)
  {
    ...
    cat(paste(res, collapse = ", "), file = out_file, append = TRUE, sep = "\n")
    return(res)
  }
...
abc_rej = ABC_rejection(model = retry_wrapper,
                        ...
                        use_seed = TRUE,
                        verbose = TRUE)
save(abc_rej, abc_param_names, file = "out_rej_sd_134.rdata")
