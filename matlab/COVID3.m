...
% de(2)
% version 3: age-specific diagnostic rates, simple deaths 
function z=COVID3(t,x)
...
%bb=b(1:4); % NO social distancing (Sc.2)
...
%dd=d(1); % NO increase in diagnosis (Sc.3)
for ag=0:(agG-1)
...
z(agC*ag+5)=ga(2)*x(agC*ag+4)-(dd(ag+1)+(1-m(ag+1))*h(ag+1)+m(ag+1)*r(2))*x(agC*ag+5);

%6 - diagnosed infected ages (0-19,20-49,50-69,70+)
z(agC*ag+6)=dd(ag+1)*x(agC*ag+5)-((1-m(ag+1))*h(ag+1)+m(ag+1)*r(2))*x(agC*ag+6);

%7 - hospitalized ages (0-19,20-49,50-69,70+)
z(agC*ag+7)=(1-m(ag+1))*h(ag+1)*(x(agC*ag+5)+x(agC*ag+6))...
           -(f(ag+1,1)+r(3))*x(agC*ag+7);%simple deaths (version 3)
%z(agC*ag+7)=(1-m(ag+1))*h(ag+1)*(x(agC*ag+5)+x(agC*ag+6))...       
...
z(agC*agG+agO*ag+3)=(dd(ag+1)+(1-m(ag+1))*h(ag+1))*x(agC*ag+5);

% cumulative deaths ages (0-19,20-49,50-69,70+)
z(agC*agG+agO*ag+4)=f(ag+1,1)*x(agC*ag+7); %simple deaths (version 3)
