...
foreach(i=1:length(sdmins)) %dopar%
{
      ...
      params_school_temp = append(scenarios_temp[["B"]], school_0)
	...
	save(scen_out, wide_out, file = paste0("big_out/scenario0_NoWane_SD_", sdmins[i], "_30_data.rdata"))
...
foreach(i=1:length(sdmins)) %dopar%
{
      sdmin_scenario = scenarios[["B"]]
      sdmin_scenario$dynamic_sd_min = c(sdmins[i]/100,sdmins[i]/100,sdmins[i]/100,2*sdmins[i]/100)
      params_school_temp = append(scenarios_temp[["B"]], school_50)
	scen_out = get_model_data_param_sets(param_set, param_names, sdmin_scenario, params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario50_NoWane_SD_", sdmins[i], "_30_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
...
foreach(i=1:length(sdmins)) %dopar%
{
      sdmin_scenario = scenarios[["B"]]
      sdmin_scenario$dynamic_sd_min = c(sdmins[i]/100,sdmins[i]/100,sdmins[i]/100,2*sdmins[i]/100)
      params_school_temp = append(scenarios_temp[["B"]], school_75)
	scen_out = get_model_data_param_sets(param_set, param_names, sdmin_scenario, params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario75_NoWane_SD_", sdmins[i], "_30_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
...
foreach(i=1:length(sdmins)) %dopar%
{
      sdmin_scenario = scenarios[["B"]]
      sdmin_scenario$dynamic_sd_min = c(sdmins[i]/100,sdmins[i]/100,sdmins[i]/100,2*sdmins[i]/100)
      params_school_temp = append(scenarios_temp[["B"]], school_100)
	scen_out = get_model_data_param_sets(param_set, param_names, sdmin_scenario, params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario100_NoWane_SD_", sdmins[i], "_30_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
