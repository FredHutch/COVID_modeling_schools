...
foreach(i=1:length(scenarios)) %dopar%
{
	...
	save(scen_out, wide_out, file = paste0("big_out/scenario0_Wane_", names(scenarios)[i], "_data.rdata"))
...
foreach(i=1:length(scenarios)) %dopar%
{
	...
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             ...
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario50_Wane_", names(scenarios)[i], "_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
...
foreach(i=1:length(scenarios)) %dopar%
{
	...
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             ...
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario75_Wane_", names(scenarios)[i], "_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
...
foreach(i=1:length(scenarios)) %dopar%
{
	...
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             ...
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE) 
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))
	save(scen_out, wide_out, file = paste0("big_out/scenario100_Wane_", names(scenarios)[i], "_data.rdata"))
	rm(list = c("scen_out", "wide_out"))
