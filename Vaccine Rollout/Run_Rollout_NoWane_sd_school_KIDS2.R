...
{ 
      params_school_temp = append(scenarios_temp_kids2[[i]], school_0)
	...
	save(scen_out, wide_out, file = paste0("big_out/scenario0_KIDS2_NoWane_", names(scenarios)[i], "_data.rdata"))   
	rm(list = c("scen_out", "wide_out"))   
	print(paste("scenario 0 kids2", names(scenarios)[i]))        
}
foreach(i=1:length(scenarios)) %dopar%
{ 
      params_school_temp = append(scenarios_temp_kids2[[i]], school_50)
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE)  
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))  
	save(scen_out, wide_out, file = paste0("big_out/scenario50_KIDS2_NoWane_", names(scenarios)[i], "_data.rdata"))   
	rm(list = c("scen_out", "wide_out"))   
	print(paste("scenario 50 kids2", names(scenarios)[i]))        
}
foreach(i=1:length(scenarios)) %dopar%
{ 
      params_school_temp = append(scenarios_temp_kids2[[i]], school_75)
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE)  
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))  
	save(scen_out, wide_out, file = paste0("big_out/scenario75_KIDS2_NoWane_", names(scenarios)[i], "_data.rdata"))   
	rm(list = c("scen_out", "wide_out"))   
	print(paste("scenario 75 kids2", names(scenarios)[i]))        
}
foreach(i=1:length(scenarios)) %dopar%
{ 
      params_school_temp = append(scenarios_temp_kids2[[i]], school_100)
	scen_out = get_model_data_param_sets(param_set, param_names, scenarios[[i]], params_school_temp,
                             state_scenario_base, NULL, start_date = ymd("2021-06-01"), end_date = ymd("2022-06-01"),
                             calc_r_eff = TRUE, out_type = "long", parallel = TRUE)  
	wide_out = lapply(scen_out, function(x) shape_data_wide(x))  
	save(scen_out, wide_out, file = paste0("big_out/scenario100_KIDS2_NoWane_", names(scenarios)[i], "_data.rdata"))   
	rm(list = c("scen_out", "wide_out"))   
	print(paste("scenario 100 kids2", names(scenarios)[i]))        
}
