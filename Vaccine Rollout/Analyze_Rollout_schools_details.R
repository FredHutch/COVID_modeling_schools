...
pdf("out/Wane_scenario_schools_SD_10_30_detail.pdf", width = 8, height = 8)
par(mfrow = c(2,2))
# vaccine coverage
...
for(scens in 1:(length(school_scenario_names))) #%dopar%
{
  load(file = paste0("big_out/scenario",school_scenario_names[scens],"_KIDS_Wane_B_data.rdata"))   
  out_kids_long[[scens]] = scen_out
  rm(list = c("scen_out", "wide_out"))   
  gc()
}

pdf("out/Wane_scenario_schools_KIDS_SD_10_30_details.pdf", width = 8, height = 8)
par(mfrow = c(2,2))
# vaccine coverage
for (scen in 1:length(school_scenario_names))
{
...
for(scens in 1:(length(school_scenario_names))) #%dopar%
{
  load(file = paste0("big_out/scenario",school_scenario_names[scens],"_KIDS2_Wane_B_data.rdata"))   
  out_kids_long[[scens]] = scen_out
  rm(list = c("scen_out", "wide_out"))   
  gc()
}

pdf("out/Wane_scenario_schools_KIDS2_SD_10_30_details.pdf", width = 8, height = 8)
par(mfrow = c(2,2))
# vaccine coverage
for (scen in 1:length(school_scenario_names))
{
