...
```{r analysis, echo = F, warning = F, message = F, cache = T}
GAMMA = 0.6
source("../Data Exploration Scripts/Estimate_HFR.R")
source("../Data Exploration Scripts/Analyze_Diagnostic_Delay.R")
source("../Data Exploration Scripts/Estimate_Severe_Cases.R")
source("../Data Exploration Scripts/Estimate_Diagnostic_Rate.R")
source("../Data Exploration Scripts/Estimate_True_Infections.R")
source("../Data Exploration Scripts/Plot_Diagnostic_Delay.R")
source("../Data Exploration Scripts/Plot_Diagnosis_Rate.R")

working_folder = "../WA Data Exploration/"
MONTHS_TO_ANALYZE = seq(3, 15)

HFR.out = Estimate_HFR(working_folder = working_folder, MONTHS_TO_ANALYZE = MONTHS_TO_ANALYZE)
DIAG.out = Estimate_Diagnostic_Rate(working_folder = working_folder, MONTHS_TO_ANALYZE = MONTHS_TO_ANALYZE)
SEV.out = Estimate_Severe_Cases(working_folder = working_folder, MONTHS_TO_ANALYZE = MONTHS_TO_ANALYZE)
DELAY.out = Analyze_Diagnostic_Delay(working_folder = working_folder, MONTHS_TO_ANALYZE = MONTHS_TO_ANALYZE)

Plot_Diagnostic_Rate(working_folder = working_folder, MONTHS_TO_PLOT = MONTHS_TO_ANALYZE)
Plot_Diagnostic_Delay(working_folder = working_folder)
```
...
DATA_AGGR_WEEKLY$MONTH = month(DATA_AGGR_WEEKLY$STARTDT) + 12 * (year(DATA_AGGR_WEEKLY$STARTDT) - 2020)
COMBINED_DATA_RESULTS = merge(monthly.rates, DATA_AGGR_WEEKLY)
```
...
output.plot = ddply(diagnostic.rates, .var = c("SEVERITY", "MONTH"), get.monthly.table)

knitr::kable(output.plot[,-2], booktabs = TRUE, caption = "Diagnostic rate (1/day)")
```
...
```{r SEVPlot, echo = F, fig.cap = "Hospitalization fraction vs week. Dots show the proportion of symptomatic infections in a particular week which result in hospitalization and/or death. Line shows the monthly fit from logistic regression."}
MONTHS_TO_SHOW = seq(3, 13)
